...
module JsonVerifyTests = 
    ...
    [<Fact>]
    let ``true = true``() = 
        let diffs = JsonVerify.Diff("true", "true")
        Assert.Empty(diffs)

    [<Fact>]
    let ``false = false``() = 
        let diffs = JsonVerify.Diff("false", "false")
        Assert.Empty(diffs)

    [<Fact>]
    let ``0 = 0``() = 
        let diffs = JsonVerify.Diff("0", "0")
        Assert.Empty(diffs)
    ...
    [<Fact>]
    let ``1000 = 1E3``() = 
        let diffs = JsonVerify.Diff("1000", "1E3")
        Assert.Empty(diffs)
        
    [<Fact>]
    let ``123.4 = 1.234E2``() = 
        let diffs = JsonVerify.Diff("123.4", "1.234E2")
        Assert.Empty(diffs)

    [<Fact>]
    let ``1 != 2 : number value mismatch``() = 
        let diffMessage = JsonVerify.Diff("1", "2") |> Seq.head
        Assert.Equal("Number value mismatch at $.\nExpected 2 but was 1.", diffMessage)
        
    [<Fact>]
    let ``1 != 1E1 : number value mismatch``() = 
        let diffMessage = JsonVerify.Diff("1", "1E1") |> Seq.head
        Assert.Equal("Number value mismatch at $.\nExpected 1E1 but was 1.", diffMessage)
        
    [<Fact>]
    let ``1 = 1.0``() =
        let diffs = JsonVerify.Diff("1", "1.0")
        Assert.Empty(diffs)

    [<Fact>]
    let ``true != 1 : Kind mismatch``() =
        let diffs = JsonVerify.Diff("true", "1") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``"foo" = "foo"``() = 
        let diffs = JsonVerify.Diff("\"foo\"", "\"foo\"")
        Assert.Empty(diffs)

    [<Fact>]
    let ``"foo" != "bar"``() = 
        let diffs = JsonVerify.Diff("\"foo\"", "\"bar\"") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``[ 1 ] vs [ 2 ]``() = 
        let diffs = JsonVerify.Diff("[ 1 ]", "[ 2 ]") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``[ 1 ] vs [ 1, 2 ]``() =
        let diffs = JsonVerify.Diff("[ 1 ]", "[ 1, 2 ]") |> Seq.toList
        match diffs with 
    ...
    let ``[ 1 ] != [ 2, 1 ] : array length mismatch``() = 
        let diffs = JsonVerify.Diff("[ 1 ]", "[ 2, 1 ]") |> Seq.toList
        match diffs with
        | [ diffMessage ] -> Assert.Equal("Array length mismatch at $.\nExpected 2 items but was 1.", diffMessage)
        | _ -> failwith "Wrong number of diffs"
    ...
    [<Fact>]
    let ``[ 2, 1 ] vs [ 1, 2 ]``() =
        let diffs = JsonVerify.Diff("[ 2, 1 ]", "[ 1, 2 ]") |> Seq.toList
        match diffs with
        | [ diff1; diff2 ] ->
    ...
    [<Fact>]
    let ``{} != { "count": 0 }``() =
        let diffs = JsonVerify.Diff("{}", "{ \"count\": 0 }") |> Seq.toList
        match diffs with
        | [ diff ] ->
    ...
    let ``{ "age": 20, "name": "Don" } = { "name": "Don", "age": 20 }``() =
        let diffs = JsonVerify.Diff("""{ "age": 20, "name": "Don" }""", """{ "name": "Don", "age": 20 }""")
        Assert.Empty(diffs)
    ...
    let ``Compare object with array``() =
        ...
        let diffs = JsonVerify.Diff(s1, s2) |> Seq.toList
        match diffs with
        | [ diff ] ->
