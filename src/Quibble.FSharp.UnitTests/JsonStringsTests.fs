...
module JsonStringsTests = 
    ...
    let ``true != 1 : Kind mismatch``() =
        let diffs = JsonStrings.Verify("true", "1") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``"foo" != "bar"``() = 
        let diffs = JsonStrings.Verify("\"foo\"", "\"bar\"") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``[ 1 ] vs [ 2 ]``() = 
        let diffs = JsonStrings.Verify("[ 1 ]", "[ 2 ]") |> Seq.toList
        match diffs with
    ...
    [<Fact>]
    let ``[ 1 ] vs [ 1, 2 ]``() =
        let diffs = JsonStrings.Verify("[ 1 ]", "[ 1, 2 ]") |> Seq.toList
        match diffs with 
    ...
    let ``[ 1 ] != [ 2, 1 ] : array length mismatch``() = 
        let diffs = JsonStrings.Verify("[ 1 ]", "[ 2, 1 ]") |> Seq.toList
        match diffs with
        | [ diffMessage ] -> Assert.Equal("Array length mismatch at $.\nExpected 2 items but was 1.", diffMessage)
        | _ -> failwith "Wrong number of diffs"
    ...
    [<Fact>]
    let ``[ 2, 1 ] vs [ 1, 2 ]``() =
        let diffs = JsonStrings.Verify("[ 2, 1 ]", "[ 1, 2 ]") |> Seq.toList
        match diffs with
        | [ diff1; diff2 ] ->
    ...
    [<Fact>]
    let ``{} != { "count": 0 }``() =
        let diffs = JsonStrings.Verify("{}", "{ \"count\": 0 }") |> Seq.toList
        match diffs with
        | [ diff ] ->
    ...
    let ``Compare object with array``() =
        ...
        let diffs = JsonStrings.Verify(s1, s2) |> Seq.toList
        match diffs with
        | [ diff ] ->
