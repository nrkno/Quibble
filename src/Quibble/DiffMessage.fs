    ...
    let toDiffMessage (diff : Diff) : string = 
        ...
        | Properties ({ Path = path; Left = actual; Right = expected} , mismatches) -> 
            ...
            let maybeAdditionalsStr = 
                if additionals.IsEmpty then None 
                else
                    let text = if List.length additionals = 1 then "property" else "properties"
                    Some <| sprintf "Additional %s:\n%s." text (String.concat "\n" additionals)
            let maybeMissingsStr = 
                if missings.IsEmpty then None
                else
        ...
        | Value { Path = path; Left = actual; Right = expected } ->
            ...
            | JsonValueKind.String -> 
                ...
                let maxStrLen = max (String.length expectedStr) (String.length actualStr)
                let comparisonStr =                 
        ...
        | ItemCount { Path = path; Left = actual; Right = expected } -> 
            ...
            let itemsStr = if expectedLength = 1 then "item" else "items"
